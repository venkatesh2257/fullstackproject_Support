...
// Get auction details with merchant info
$stmt = $conn->prepare("SELECT a.*, p.name as product_name, p.description, p.image_url, p.merchant_price, p.merchant_id, p.admin_price 
                        FROM auctions a 
...
// Check if current user is the merchant who owns this product
    ...
    if (!$has_subscription) {
        ...
        if ($bid_amount <= $auction['current_price']) {
            ...
            try {
                ...
                // Refresh auction data with product info
                $stmt = $conn->prepare("SELECT a.*, p.name as product_name, p.description, p.image_url, p.merchant_price, p.merchant_id, p.admin_price 
                                        FROM auctions a 
                                        JOIN products p ON a.product_id = p.id 
                                        WHERE a.id = ?");
                ...
                // Re-check if user is product owner
                $is_product_owner = false;
                if (getUserRole() === 'merchant' && $user_id == $auction['merchant_id']) {
                    $is_product_owner = true;
                }
        ...
        <div class="auction-detail">
            ...
            <div class="auction-info">
                ...
                <div class="auction-stats">
                    ...
                    $display_current_price = $auction['current_price'];
                    $display_starting_price = $auction['starting_price'];
                    
                    if ($is_product_owner) {
                        // Merchant viewing their own product - show merchant_price
                        $display_current_price = $auction['merchant_price'];
                        $display_starting_price = $auction['merchant_price'];
                    }
                    ?>
                    <div class="stat-item">
                        <span class="stat-label">Current Bid:</span>
                        <span class="stat-value">$<?php echo number_format($display_current_price, 2); ?></span>
                        <?php if ($is_product_owner): ?>
                            <small style="display: block; color: #50c878; margin-top: 5px;">(Your Price)</small>
                        <?php endif; ?>
                    </div>
                    <div class="stat-item">
                        <span class="stat-label">Starting Price:</span>
                        <span class="stat-value">$<?php echo number_format($display_starting_price, 2); ?></span>
                        <?php if ($is_product_owner): ?>
                            <small style="display: block; color: #50c878; margin-top: 5px;">(Your Price)</small>
                        <?php endif; ?>
                    </div>
                        ...
                        <?php else: ?>
                            <?php if ($is_product_owner): ?>
                                <div class="alert alert-info">
                                    This is your product. You cannot bid on your own product.
                                </div>
                            <?php else: ?>
